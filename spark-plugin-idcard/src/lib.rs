...
//! 成功返回 `性别 · 出生日期 · 地区`；失败返回 err（值，不是 trap）。
...
fn check_birth(id: &str) -> Result<(), String> {
...
impl Guest for Idcard {
    ...
    fn transform(input: String) -> Result<String, String> {
        let id = input.trim();
        if id.len() != 18 {
            return Err("长度错误：应为 18 位".into());
        }
        if !id[..17].chars().all(|c| c.is_ascii_digit()) {
            return Err("格式错误：前 17 位须为数字".into());
        }
        check_birth(id)?;
        if !check_sum(id) {
